...
class ChatCommandRegistry {
    ...
    register(command) {
        if (!(command instanceof ChatCommand)) {
            throw new Error('(' + command.name + ') ChatCommand must extend ConsoleCommand class');
        }
    ...
    async executeChatCommand(instance, username, message) {
        ...
        if (!await hasPermissions(username, instance.bot.username, cmd.permissionLevel) && instance.options.auth !== "offline") {
        ...
        try {
            ...
            instance.bot.whisper(username, 'An error occurred while executing the command: ' + error.message);
        }
    }
